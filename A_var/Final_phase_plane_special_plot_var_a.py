...
def get_A1(t):
    Q,R,k,Am = 1.3518,1,1,0.7
    return Am+ (k*(1 + np.tanh((Q*np.sin(0.5*np.pi*t)) -R)))

def get_A2(t):
    Q,R,k1,k2,Am = 4,3.42,1,0.5,0.7
...
def ivp_model(t,z,a=2,b=0.45,no_of_evals = num):#,a=1.1776,b=0.45,no_of_evals = num):
    #split into w and u
    u = z[:no_of_evals]
    w = z[no_of_evals:]
    #system
...
##    plt.savefig('Project_plots/phaseplane_smooth_A_2__B_0_5.png')
